...
This directory contains deployment-facing files for the Community repository:
- `docker-compose.yml`
- `.env.example`
...
The example pins tested Enterprise AI images by digest. Do not replace them
with an unverified tag.

### Private Enterprise image access

Enterprise application images are private GHCR packages. Image access is a
delivery permission, separate from the signed product license that controls
what the installation can run. A customer does not need access to Veriqorn
source repositories.

During Enterprise provisioning:

1. The customer provides the GitHub username of a dedicated deployment
   account (for example, `acme-veriqorn-deploy`). Do not use an individual
   employee account when a service account is available.
2. Veriqorn grants that account **Read** access only to the
   ...
   Package access must not grant source-repository access.
3. The customer creates a classic GitHub personal access token (PAT) with only
   the `read:packages` scope. If the organization requires SSO authorization,
   the customer authorizes that PAT for the organization.
4. Veriqorn separately issues the signed, installation-bound Enterprise
   license after activation.

The deployment account is only a way to download new image versions. It does
not replace the product license and must not receive Veriqorn owner credentials
or write access to packages.
...
when the job finishes; never commit the token or put it in `.env.enterprise`:

```env
UPDATE_REGISTRY_USERNAME=acme-veriqorn-deploy
UPDATE_REGISTRY_TOKEN=<customer PAT with read:packages>
```

After changing either value, recreate only the update agent:
...
For a token rotation, replace the customer PAT in local `.env` and run the same
command. To stop a former customer from downloading future Enterprise releases,
Veriqorn revokes that deployment account's package read access; the existing
installation-bound product license remains independently subject to its expiry
...
powershell -ExecutionPolicy Bypass -File .\bundle-airgap.ps1 `
  ...
  -Version v0.2.28 `
  -OutputDirectory .\veriqorn-v0.2.32-airgap
```
